...
const UDP_PORT = Number(process.env.F1_UDP_PORT || 20777), WS_URL = process.env.TELEMETRY_WS_URL, WRITE_KEY = process.env.TELEMETRY_WRITE_KEY, SESSION_TOKEN = process.env.TELEMETRY_SESSION_TOKEN;
...
type Header = { packetFormat: number; packetId: number; playerCarIndex: number; sessionUid: string };
...
type Track = { id: number; name: string; country: string; trackLength: number; weather: string; gameSessionUid: string };
const WEATHER_NAMES: Record<number, string> = { 0: "Clear", 1: "Light cloud", 2: "Overcast", 3: "Light rain", 4: "Heavy rain", 5: "Storm" };
...
function readHeader(packet: Buffer): Header | null { return packet.length < HEADER_SIZE ? null : { packetFormat: packet.readUInt16LE(0), packetId: packet.readUInt8(6), sessionUid: packet.readBigUInt64LE(7).toString(), playerCarIndex: packet.readUInt8(27) }; }
...
function parseSession(packet: Buffer, header: Header) { if (packet.length < HEADER_SIZE + 8) return; const id = packet.readInt8(HEADER_SIZE + 7); if (id === announcedTrack) return; announcedTrack = id; const known = TRACKS[id], trackLength = packet.readUInt16LE(HEADER_SIZE + 4), weather = WEATHER_NAMES[packet.readUInt8(HEADER_SIZE)] || "Unknown weather"; const track: Track = known ? { id, ...known, trackLength, weather, gameSessionUid: header.sessionUid } : { id, name: `Circuit ${id}`, country: "Unknown location", trackLength, weather, gameSessionUid: header.sessionUid }; console.log(`Session track: ${track.name} (${track.country})`); broadcast({ type: "session", payload: track }); }
function parseLapData(packet: Buffer, header: Header) { const car = header.playerCarIndex, offset = HEADER_SIZE + car * LAP_DATA_SIZE; if (offset + LAP_DATA_SIZE > packet.length) return; const participant = participants.get(car), history = historyByCar.get(car) ?? [], completed = history.filter((lap) => lap.valid), previous = history.at(-1), bestSector = (field: "sector1" | "sector2" | "sector3") => { const candidate = Math.min(...completed.map((lap) => lap[field]).filter(Boolean), Infinity); return Number.isFinite(candidate) ? candidate : 0; }, lastLapTime = packet.readUInt32LE(offset), currentLapTime = packet.readUInt32LE(offset + 4), currentLap = packet.readUInt8(offset + 33), sector1 = duration(packet.readUInt8(offset + 10), packet.readUInt16LE(offset + 8)), sector2 = duration(packet.readUInt8(offset + 13), packet.readUInt16LE(offset + 11)), deltaToLeader = duration(packet.readUInt8(offset + 19), packet.readUInt16LE(offset + 17)), lapDistance = packet.readFloatLE(offset + 20), lastLapValid = previous?.lapTime === lastLapTime && previous.valid; broadcast({ type: "lap", payload: { carIndex: car, position: packet.readUInt8(offset + 32), currentLap, lastCompletedLap: Math.max(0, currentLap - 1), currentLapTime, lastLapTime, sector1, sector2, sector3: previous?.lapTime === lastLapTime ? previous.sector3 : 0, personalBestSector1: bestSector("sector1"), personalBestSector2: bestSector("sector2"), personalBestSector3: bestSector("sector3"), deltaToLeader, lapDistance, teamId: participant?.teamId ?? -1, team: participant?.team ?? "Unknown team", car: participant?.team ?? "Unknown car", driverName: participant?.name ?? "PLAYER", lapValid: packet.readUInt8(offset + 37) === 0, lastLapValid: Boolean(lastLapValid), timestamp: Date.now() } }); }
let ws: WebSocket | undefined;
function connect() { const separator = WS_URL!.includes("?") ? "&" : "?"; const params = new URLSearchParams({ key: WRITE_KEY! }); if (SESSION_TOKEN) params.set("session", SESSION_TOKEN); ws = new WebSocket(`${WS_URL}${separator}${params}`); ws.on("open", () => console.log(`Connected to telemetry relay (${WS_URL})${SESSION_TOKEN ? " with verified account link" : ""}`)); ws.on("close", () => { console.log("Relay disconnected; retrying in 2 seconds…"); setTimeout(connect, 2000); }); ws.on("error", () => ws?.close()); }
...
udp.on("message", (packet) => {
  ...
  if (header.packetId === 1) parseSession(packet, header);
